...
pub(crate) fn build_glm_request(
    config: &AgentConfig,
    history: Vec<Message>,
    ...
    if let Some(s) = &config.system_prompt
        && !s.is_empty()
    {
        messages.push(GlmMessage::System { content: s.clone() });
    }
    for m in history {
        match m {
            Message::User(parts) => messages.push(GlmMessage::User {
            ...
            Message::ToolResult { call_id, content } => {
                ...
                let tool_content = if let [Content::Text { text }] = content.as_slice() {
                    ...
                    // GLM chat-completions doesn't accept file parts —
...
fn user_content_from_parts(parts: Vec<UserContent>) -> UserMessageContent {
    let blocks: Vec<ContentPart> = parts
        ...
        .filter_map(|p| match p {
            ...
            // GLM has no wire for files — silently drop.
