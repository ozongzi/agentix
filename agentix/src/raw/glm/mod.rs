...
use request::build_glm_request;
use response::{DeltaToolCall, StreamChunk};

pub(crate) async fn stream_glm(
    token: &str,
    http: &reqwest::Client,
    config: &AgentConfig,
    messages: &[Message],
    ...
    let req = build_glm_request(config, messages.to_vec(), tools, tool_choice, true);
            ...
            match ev_res {
                ...
                Ok(ev) => {
                    ...
                    match serde_json::from_str::<StreamChunk>(&ev.data) {
                        ...
                        Err(e)    => debug!(data = %ev.data, error = %e, "glm chunk parse failed"),
                    }
                }
...
pub(crate) async fn complete_glm(
    token: &str,
    http: &reqwest::Client,
    config: &AgentConfig,
    messages: &[Message],
    ...
    let req = build_glm_request(config, messages.to_vec(), tools, tool_choice, false);
    let url = format!("{}/chat/completions", config.base_url.trim_end_matches('/'));
    let body = post_json(
        http,
        &url,
