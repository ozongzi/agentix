...
pub(crate) fn build_kimi_request(
    config: &AgentConfig,
    history: Vec<Message>,
    ...
    if let Some(s) = &config.system_prompt
        && !s.is_empty()
    {
        messages.push(KimiMessage::System { content: s.clone() });
    }
    for m in history {
        match m {
            Message::User(parts) => messages.push(KimiMessage::User {
            ...
            Message::ToolResult { call_id, content } => {
                ...
                let tool_content = if let [Content::Text { text }] = content.as_slice() {
                    ...
                    // Kimi chat-completions doesn't accept file parts —
...
fn user_content_from_parts(parts: Vec<UserContent>) -> UserMessageContent {
    let blocks: Vec<ContentPart> = parts
        ...
        .filter_map(|p| match p {
            ...
            // Kimi has no wire for files — silently drop.
