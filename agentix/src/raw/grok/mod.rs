...
pub(crate) async fn stream_grok(
    token:    &str,
    http:     &reqwest::Client,
    config:   &AgentConfig,
    messages: &[Message],
    ...
    let req = build_grok_request(config, messages.to_vec(), tools, tool_choice, true);
        ...
        while let Some(ev_res) = sse.next().await {
            match ev_res {
                ...
                Ok(ev) => match serde_json::from_str::<StreamChunk>(&ev.data) {
                    ...
                    Err(e)    => debug!(data = %ev.data, error = %e, "grok chunk parse failed"),
                },
...
pub(crate) async fn complete_grok(
    token:    &str,
    http:     &reqwest::Client,
    config:   &AgentConfig,
    messages: &[Message],
    ...
    let tool_choice = if tools.is_empty() { None } else { Some(ToolChoice::Auto) };
    let req = build_grok_request(config, messages.to_vec(), tools, tool_choice, false);
    let url = format!("{}/chat/completions", config.base_url.trim_end_matches('/'));
